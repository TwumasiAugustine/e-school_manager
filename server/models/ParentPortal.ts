...
const progressReportSchema = new Schema<IProgressReport>({
	...
	subjects: {
		...
		required: true,
		validate: [
			(v: ISubjectGrade[]) => Array.isArray(v) && v.length > 0,
...
const parentPortalSchema = new Schema<IParentPortal>(
	{
		...
		students: {
			...
			required: true,
			validate: [
				(v: ILinkedStudent[]) => Array.isArray(v) && v.length > 0,
...
parentPortalSchema.index({
	'feePayments.student': 1,
	'feePayments.term': 1,
	'feePayments.academicYear': 1,
});
parentPortalSchema.index({ 'attendance.student': 1, 'attendance.date': 1 });
parentPortalSchema.index({
	'progressReports.student': 1,
	'progressReports.term': 1,
	'progressReports.academicYear': 1,
});
parentPortalSchema.index({
...
const ParentPortal = mongoose.model<IParentPortal>(
	'ParentPortal',
	parentPortalSchema,
);
